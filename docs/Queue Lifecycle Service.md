...
- `retry_task(workflow_id, task_id, attempt, error)` records
  `task_attempt_failed`, requeues the claimed task, then records `task_queued`.
...
- The service does not execute tasks; [[Worker]] decides whether the task's
  retry budget permits `retry_task` or requires terminal failure.
